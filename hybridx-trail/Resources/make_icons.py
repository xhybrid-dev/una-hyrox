#!/usr/bin/env python3
"""Draw the Trail Probe's launcher icons: a plain white "P" on a dark green tile.
...
ABGR2222 (two bits per channel, alpha included). This probe is throwaway (T0,
never released), so the icon is placeholder artwork only -- legible enough to
tell it apart on the watch's app list, nothing more. The real HybridX Trail app
will get its own artwork in its design phase.
...
TEAL = (0, 170, 85, 255)        # VIRIDIAN   0x008040
TEAL_DARK = (0, 85, 0, 255)     # GREEN_DARK 0x004000
...
def draw(size):
    ...
    # A rounded dark-green tile, with a plain white "P" for "Probe".
